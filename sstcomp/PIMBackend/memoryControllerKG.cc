...
/*************************** Memory Controller ********************/
MemControllerKG::MemControllerKG(ComponentId_t id, Params &params)
    : Component(id), backing_(NULL) {

  kgdbg::spinner("MEM_SPINNER", getName().compare("memory0") == 0);

  dlevel = params.find<unsigned>("debug_level", 0);

  fixupParams(params, "backend", "backendConvertor.backend");
  fixupParams(params, "backend.", "backendConvertor.backend.");
  fixupParams(params, "request_width", "backendConvertor.request_width");
  fixupParams(params, "max_requests_per_cycle",
              "backendConvertor.backend.max_requests_per_cycle");

  uint32_t requestWidth =
      params.find<uint32_t>("backendConvertor.request_width", 64);
  ...
  for (std::vector<Addr>::iterator it = addrArr.begin(); it != addrArr.end();
       it++) {
    DEBUG_ADDR.insert(*it);
  }

  // Output for warnings
  out.init("", params.find<unsigned>("verbose", 1), 0, Output::STDOUT);
  ...
  if (!(clock_ua.hasUnits("Hz") || clock_ua.hasUnits("s")) ||
    ...
    out.fatal(CALL_INFO, -1,
              "%s, ERROR - Invalid parameter: 'clock'. Must have units of Hz "
              "or s and be > 0. (SI prefixes ok). You specified '%s'\n",
              getName().c_str(), clockfreq.c_str());
  }

  clockHandler_ =
      new Clock::Handler2<MemControllerKG, &MemControllerKG::clock>(this);
  ...
  /* MemControllerKG supports multiple ways of loading in backends:
   *  Legacy:
   *      Define backend and/or backendConvertor in the memcontroller's
   * parameter set Better way: Fill backend slot with backend and memcontroller
   * loads the compatible convertor
   *
   */
  ...
  if (!memory) { /* Try to load from our parameters (legacy mode 1) */
    ...
    if (info && info->isPopulated(0)) {
      out.fatal(CALL_INFO, -1,
                "%s, ERROR: Unable to load the subcomponent in the 'backend' "
                "slot. Check that the requested subcomponent is registered "
                "with the SST core.\n",
                getName().c_str());
    } else {
      out.output("%s, WARNING: Loaded backend in legacy mode (from parameter "
                 "set). Instead, load backend into this controller's 'backend' "
                 "slot via ctrl.setSubComponent() in configuration.\n",
                 getName().c_str());
    }
    Params tmpParams = params.get_scoped_params("backendConvertor.backend");
    std::string name = params.find<std::string>("backendConvertor.backend",
                                                "memHierarchy.simpleMem");
    memory_ = loadAnonymousSubComponent<MemBackend>(
        name, "backend", 0,
        ComponentInfo::INSERT_STATS | ComponentInfo::SHARE_PORTS, tmpParams);
    if (!memory_) {
      out.fatal(CALL_INFO, -1,
                "%s, ERROR: Unable to load backend '%s'. Use setSubComponent() "
                "on this controller to specify backend in your input "
                "configuration; check for valid backend name.\n",
                getName().c_str(), name.c_str());
    }
  }

  std::string convertortype = memory_->getBackendConvertorType();
  Params tmpParams = params.get_scoped_params("backendConvertor");
  memBackendConvertor_ = loadAnonymousSubComponent<MemBackendConvertor>(
      convertortype, "backendConvertor", 0, ComponentInfo::INSERT_STATS,
      tmpParams, memory_, requestWidth);

  if (memBackendConvertor_ == nullptr) {
    out.fatal(CALL_INFO, -1, "%s, ERROR: Unable to load MemBackendConvertor.",
              getName().c_str());
  }
  ...
  memBackendConvertor_->setCallbackHandlers(
      std::bind(&MemControllerKG::handleMemResponse, this, _1, _2),
      std::bind(&MemControllerKG::turnClockOn, this));
  memSize_ = memBackendConvertor_->getMemSize();
  if (memSize_ == 0)
    out.fatal(CALL_INFO, -1,
              "%s, ERROR: Tried to get memory size from backend but size is "
              "0B. Either backend is missing a 'mem_size' parameter or value "
              "is invalid.\n",
              getName().c_str());

  // Load listeners (profilers/tracers/etc.)
  SubComponentSlotInfo *lists = getSubComponentSlotInfo(
      "listener"); // Find all listeners specified in the configuration
  if (lists) {
    for (int i = 0; i < lists->getMaxPopulatedSlotNumber(); i++) {
      if (lists->isPopulated(i))
        listeners_.push_back(
            lists->create<CacheListener>(i, ComponentInfo::SHARE_NONE));
    }
    // lists->createAll<CacheListener>(listeners_, false,
    ...
    for (size_t i = 0; i < listenerCount; ++i) {
      snprintf(nextListenerName, bufferSize, "listener%zu", i);
      string listenerMod = params.find<std::string>(nextListenerName, "");

      if (listenerMod != "") {
        snprintf(nextListenerParams, bufferSize, "listener%zu", i);
        Params listenerParams = params.get_scoped_params(nextListenerParams);

        CacheListener *loadedListener =
            loadAnonymousSubComponent<CacheListener>(
                listenerMod, "listener", (int)i, ComponentInfo::INSERT_STATS,
                listenerParams);
  ...
  uint64_t addrEnd =
      params.find<uint64_t>("addr_range_end", (uint64_t)-1, found);
  gotRegion |= found;
  ...
  if (!UnitAlgebra(ilSize).hasUnits("B")) {
    dbg.fatal(
        CALL_INFO, -1,
        "Invalid param(%s): interleave_size - must be specified in bytes with "
        "units (SI units OK). For example, '1KiB'. You specified '%s'\n",
  ...
  if (!UnitAlgebra(ilStep).hasUnits("B")) {
    dbg.fatal(
        CALL_INFO, -1,
        "Invalid param(%s): interleave_step - must be specified in bytes with "
        "units (SI units OK). For example, '1KiB'. You specified '%s'\n",
        getName().c_str(), ilSize.c_str());
  }
  ...
  region_.end = addrEnd;
  region_.interleaveSize = (uint64_t)UnitAlgebra(ilSize).getRoundedValue();
  region_.interleaveStep = (uint64_t)UnitAlgebra(ilStep).getRoundedValue();

  link_ = loadUserSubComponent<MemLinkBase>(
      "highlink", ComponentInfo::SHARE_NONE, &clockTimeBase_);
  if (!link_) {
    link_ = loadUserSubComponent<MemLinkBase>(
        "cpulink", ComponentInfo::SHARE_NONE, &clockTimeBase_);
    if (link_) {
      out.output("%s, WARNING - Deprecation: The 'cpulink' subcomponent slot "
                 "has been renamed to 'highlink' to improve name "
                 "standardization. Please change this in your input file.\n",
                 getName().c_str());
    }
  }

  flink_ = loadUserSubComponent<MemLinkBase>(
      "forwardlink", ComponentInfo::SHARE_NONE, &clockTimeBase_);
  ...
  if (!link_ && isPortConnected("direct_link")) {
    ...
    link_ = loadAnonymousSubComponent<MemLinkBase>(
        "memHierarchy.MemLink", "highlink", 0,
        ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, linkParams,
        &clockTimeBase_);
    out.output("%s, WARNING - Deprecation: To standardize port names across "
               "memHierarchy elements, the MemController's port 'direct_link' "
               "has been renamed to 'highlink'. The 'direct_link' port will be "
               "removed in SST 16.\n",
               getName().c_str());
    ...
    if (!isPortConnected("network")) {
      out.fatal(
          CALL_INFO, -1,
          "%s, ERROR: No connected port detected. Connect 'highlink' port, or "
          "if this memory is on a network, fill the 'highlink' subcomponent "
          "slot with memHierarchy.MemNIC or memHierarchy.MemNICFour.\n",
          getName().c_str());
    }

    out.output(
        "%s, WARNING - Deprecation: Use of the network* ports in MemController "
        "is deprecated. Instead, fill this component's 'highlink' subcomponent "
        "slot with memHierarchy.MemNIC or memHierarchy.MemNICFour\n",
        getName().c_str());
    ...
    if (isPortConnected("network_ack") && isPortConnected("network_fwd") &&
        isPortConnected("network_data")) {
      ...
      link_ = loadAnonymousSubComponent<MemLinkBase>(
          "memHierarchy.MemNICFour", "highlink", 0,
          ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, nicParams,
          &clockTimeBase_);
    } else {
      nicParams.insert("port", "network");
      link_ = loadAnonymousSubComponent<MemLinkBase>(
          "memHierarchy.MemNIC", "highlink", 0,
          ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, nicParams,
          &clockTimeBase_);
    }
  }
  ...
  link_->setRecvHandler(
      new Event::Handler2<MemControllerKG, &MemControllerKG::handleEvent>(
          this));

  link_->setRegion(region_);

  if (flink_) {
    fClockLink_ = flink_->isClocked();
    flink_->setRecvHandler(
        new Event::Handler2<MemControllerKG,
                            &MemControllerKG::handleFLinkEvent>(this));
    // flink is the compliment of the addresses owned by this memory controller
    // flink_->setRegion(fregion_);
  }
  ...
  if (!found) {
    ...
    if (found) {
      out.output(
          "%s, ** Found deprecated parameter: do_not_back ** Use 'backing' "
          "parameter instead and specify 'none', 'malloc', or 'mmap'. Remove "
          "this parameter from your input deck to eliminate this message.\n",
          getName().c_str());
    }
    if (oldBackVal)
      backingType = "none";
  ...
  if (backingType != "none" && backingType != "mmap" &&
      backingType != "malloc") {
    out.fatal(CALL_INFO, -1,
              "%s, ERROR - Invalid parameter: 'backing'. Must be one of "
              "'none', 'malloc', or 'mmap'. You specified: %s\n",
  ...
  if (!size_ua.hasUnits("B")) {
    out.fatal(CALL_INFO, -1,
              "%s, ERROR - Invalid parameter: 'backing_size_unit'. Must have "
              "units of bytes (B). SI ok. You specified: %s\n",
              getName().c_str(), size.c_str());
  }
  size_t sizeBytes =
      (size_t)size_ua.getRoundedValue(); // TODO does precision loss affect us?

  if (sizeBytes > memBackendConvertor_->getMemSize()) {
    sizeBytes = memBackendConvertor_->getMemSize();
    // Since getMemSize() might not be a power of 2, but malloc store needs
    // it....get a reasonably close power of 2
    sizeBytes = 1 << log2Of((int)memBackendConvertor_->getMemSize());
  }
  ...
  if (backingType == "mmap") {
    if (!found) { // memory_file deprecated in SST 15, remove SST 16
      ...
      if (found) {
        out.output("%s, WARNING - Deprecation: The parameter 'memory_file' is "
                   "deprecated. Use 'backing_in_file' and/or "
                   "'backing_out_file' instead.",
                   getName().c_str());
    ...
    if (backing_outfile_ != infile && infile != "")
      backing_outfile_ = SST::Util::Filesystem::getAbsolutePath(
          backing_outfile_, getOutputDirectory());
    try {
      backing_ = new Backend::BackingMMAP(backing_outfile_, infile,
                                          memBackendConvertor_->getMemSize());
    } catch (int e) {
      if (e == 1)
        out.fatal(CALL_INFO, -1,
                  "%s, ERROR: Unable to open backing_out_file. Requested file "
                  "is '%s'.\n",
                  getName().c_str(), backing_outfile_.c_str());
      else if (e == 3)
        out.fatal(CALL_INFO, -1,
                  "%s, ERROR: Unable to open backing_in_file. Requested file "
                  "is '%s'.\n",
                  getName().c_str(), infile.c_str());
      else if (e == 2) {
        if (backing_outfile_ == "" && infile == "") {
          out.verbose(CALL_INFO, 1, 0,
                      "%s, WARNING: Could not MMAP backing store (likely, "
                      "simulated memory exceeds available memory space). "
                      "Creating malloc based store instead.\n",
                      getName().c_str());
          backing_ = new Backend::BackingMalloc(sizeBytes, initBacking);
        } else if (infile != "") {
          out.fatal(
              CALL_INFO, -1,
              "%s, ERROR: Could not MMAP backing store (likely, simulated "
              "memory exceeds available memory). Cannot initialize malloc "
              "based store from provided mmap input file %s.\n",
              getName().c_str(), infile.c_str());
        } else {
          out.fatal(CALL_INFO, -1,
                    "%s, ERROR: Could not MMAP backing store from file %s\n",
                    getName().c_str(), backing_outfile_.c_str());
        }
      } else if (e == 4) {
        ...
        out.fatal(CALL_INFO, -1,
                  "%s, ERROR: Unable to create backing store. Exception thrown "
                  "is %d.\n",
                  getName().c_str(), e);
    }
    ...
    if (infile != "") {
      try {
        ...
        if (e == 1) {
          out.fatal(CALL_INFO, -1,
                    "%s, ERROR: Unable to open 'backing_in_file'. Does file "
                    "exist? Filename='%s'\n",
                    getName().c_str(), infile.c_str());
        } else
          out.fatal(CALL_INFO, -1,
                    "%s, ERROR: Unable to create backing store. Exception "
                    "thrown is %d.\n",
                    getName().c_str(), e);
      }
    } else {
    ...
    if (backing_outfile_ != "") {
      ...
      sst_assert(fp, CALL_INFO, -1,
                 "%s, ERROR: Unable to open 'backing_out_file'. Is filepath "
                 "accessible? Filename='%s'\n",
                 getName().c_str(), backing_outfile_.c_str());
      fclose(fp);
    }
  } else {
  ...
  customCommandHandler_ = loadUserSubComponent<CustomCmdMemHandler>(
      "customCmdHandler", ComponentInfo::SHARE_NONE,
      std::bind(static_cast<void (MemControllerKG::*)(Addr, size_t,
                                                      std::vector<uint8_t> &)>(
                    &MemControllerKG::readData),
                this, _1, _2, _3),
      std::bind(
          static_cast<void (MemControllerKG::*)(Addr, std::vector<uint8_t> *)>(
              &MemControllerKG::writeData),
          this, _1, _2));
  if (nullptr == customCommandHandler_) {
    std::string customHandlerName =
        params.find<std::string>("customCmdHandler", "");
    if (customHandlerName != "") {
      customCommandHandler_ = loadAnonymousSubComponent<CustomCmdMemHandler>(
          customHandlerName, "customCmdHandler", 0, ComponentInfo::INSERT_STATS,
          params,
          std::bind(static_cast<void (MemControllerKG::*)(
                        Addr, size_t, std::vector<uint8_t> &)>(
                        &MemControllerKG::readData),
                    this, _1, _2, _3),
          std::bind(
              static_cast<void (MemControllerKG::*)(
                  Addr, std::vector<uint8_t> *)>(&MemControllerKG::writeData),
              this, _1, _2));
    }
  }
}
...
void MemControllerKG::handlePIMEvent(SST::Event *event) {
  out.verbose(CALL_INFO, 3, 0, "%s, Pim Event: %s\n", getName().c_str(),
...
void MemControllerKG::handleEvent(SST::Event *event) {
  ...
  if (mem_h_is_debug_event(meb)) {
    mem_h_debug_output(
        _L3_, "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:New     (%s)\n",
  ...
  if (ev->queryFlag(PIMMemEvent::F_PIM)) {
    if (!region_.contains(ev->getBaseAddr())) {
      ...
      out.verbose(
        ...
        2,
        0,
        "%s, forwarding event with base address that does not map "
  ...
  if (ev->queryFlag(PIMMemEvent::F_REMOTE)) {
    if (cmd == Command::WriteResp or cmd == Command::GetSResp) {
      out.verbose(CALL_INFO, 2, 0, "%s, Received remote response %s\n",
                  getName().c_str(), ev->getVerboseString((int)dlevel).c_str());
      static_cast<PIM::PIMBackend *>(memory_)->handlePIMCompletion(ev);
      return;
    }
  }
  ...
  if (region_.interleaveStep != 0 &&
      ev->getSize() > 0) { // Non-contiguous address region so make sure this
                           // request doesn't fall outside the region
    Addr a0 = (chkAddr - region_.start) / region_.interleaveStep;
    a0 = a0 * region_.interleaveStep +
         region_.start; // Address of the interleave chunk that contains the
                        // target address
    Addr b0 = a0 + region_.interleaveSize -
              1; // Last address in the interleave chunk that contains the
                 // target starting address
    Addr a1 =
        a0 + region_.interleaveStep; // Address of the next interleave chunk

    // If the request size is larger than one of our interleaved chunks
    // then the only way for it to completely fall in our region is if our
    // interleaving is contiguous (e.g., not reall interleaving)
      ...
      if ((b0 + 1) != a1) {
        out.fatal(CALL_INFO, -1,
                  ...
                  getName().c_str(), ev->getVerboseString(dlevel).c_str());
                                  ...
                                  // address of request falls in region
  ...
  if (ev->isAddrGlobal()) {
    ...
    // TODO seems like we clear global flag here
  }
...
#pragma GCC diagnostic ignored "-Wimplicit-fallthrough"
  ...
  case Command::Write:
    ...
    if (mem_h_is_debug_event(ev)) {
      mem_h_debug_output(
          _L4_, "B: %-20" PRIu64 " %-20" PRIu64 " %-20s Bkend:Send    (%s)\n",
          getCurrentSimCycle(), getNextClockCycle(clockTimeBase_) - 1,
  ...
  case Command::FlushLineInv: {
    ...
    if (ev->getPayloadSize() != 0) {
      put = new MemEvent(getName(), ev->getBaseAddr(), ev->getBaseAddr(),
                         Command::PutM, ev->getPayload());
      ...
      if (mem_h_is_debug_event(put)) {
        mem_h_debug_output(
            _L4_, "B: %-20" PRIu64 " %-20" PRIu64 " %-20s Bkend:Send    (%s)\n",
            getCurrentSimCycle(), getNextClockCycle(clockTimeBase_) - 1,
            getName().c_str(), put->getVerboseString().c_str());
      }
  ...
  case Command::FlushAll: {
    ...
    if (mem_h_is_debug_event(resp)) {
      mem_h_debug_output(
          _L4_, "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:Send    (%s)\n",
          getCurrentSimCycle(), getNextClockCycle(clockTimeBase_) - 1,
...
bool MemControllerKG::clock(Cycle_t cycle) {
  ...
  bool unclockflink = true;
  if (fClockLink_) {
    unclockflink = flink_->clock();
  }

  bool unclockBack = memBackendConvertor_->clock(cycle);

  if (unclockLink && unclockflink && unclockBack) {
...
Cycle_t MemControllerKG::turnClockOn() {
...
void MemControllerKG::handleCustomEvent(MemEventBase *ev) {
  if (!customCommandHandler_)
    out.fatal(CALL_INFO, -1,
              "%s, ERROR: Received custom event but no handler loaded. Ev = "
              "%s. Time = %" PRIu64 "ns\n",
              getName().c_str(), ev->getVerboseString((int)dlevel).c_str(),
  ...
  if (evInfo.shootdown) {
    out.verbose(CALL_INFO, 1, 0,
                "%s, WARNING: Custom event expects a shootdown but this memory "
                "controller does not support shootdowns. Ev = %s\n",
                getName().c_str(), ev->getVerboseString((int)dlevel).c_str());
  }
...
void SST::MemHierarchy::MemControllerKG::handleForwardedEvent(
    MemEventBase *ev) {
  assert(ev->queryFlag(PIMMemEvent::F_REMOTE) == 0);
  ev->clearFlag(PIMMemEvent::F_PIM);
  ev->setFlag(PIMMemEvent::F_REMOTE);
  // forwardedEvents_.insert( std::make_pair( ev->getID(), ev ) );

  if (flink_)
    flink_->send(ev); // dedicated NIC to node network
  else
    link_->send(ev); // shared link to local network
}

void MemControllerKG::handleMemResponse(Event::id_type id, uint32_t flags) {

  std::map<SST::Event::id_type, MemEventBase *>::iterator it =
      outstandingEvents_.find(id);
  if (it == outstandingEvents_.end())
    out.fatal(CALL_INFO, -1,
              "%s, ERROR: Memory controller received unrecognized response ID: "
              "%" PRIu64 ", %" PRIu32 "",
  ...
  if (mem_h_is_debug_event(evb)) {
    mem_h_debug_output(_L4_,
                       "B: %-20" PRIu64 " %-20" PRIu64
                       " %-20s Bkend:Recv    (<%" PRIu64 ",%" PRIu32 ">)\n",
                       getCurrentSimCycle(),
                       getNextClockCycle(clockTimeBase_) - 1, getName().c_str(),
                       id.first, id.second);
  }
  ...
  /* Write data. Here instead of receive to try to match backing access order to
   * backend execute order */
  if (backing_ &&
      (ev->getCmd() == Command::PutM || (ev->getCmd() == Command::Write))) {
    if (ev->queryFlag(PIMMemEvent::F_MMIO)) {
      ev->clearFlag(PIMMemEvent::F_MMIO);
      // TODO carry global and local address information with event so we only
      // translate once.
      if (ev->isAddrGlobal()) {
        ev->setBaseAddr(translateToGlobal(ev->getBaseAddr()));
        ev->setAddr(translateToGlobal(ev->getAddr()));
      }
      static_cast<PIM::PIMBackend *>(memory_)->handleMMIOWriteCompletion(ev);
    } else {
      writeData(ev);
    }
  }
  ...
  /* Read order matches execute order so that mis-ordering at backend can result
   * in bad data */
  if (resp->getCmd() == Command::GetSResp ||
      resp->getCmd() == Command::GetXResp) {
    if (resp->queryFlag(PIMMemEvent::F_MMIO)) {
      resp->clearFlag(PIMMemEvent::F_MMIO);
      // TODO carry global and local address information with event so we only
      // translate once.
      if (ev->isAddrGlobal()) {
      ...
      static_cast<PIM::PIMBackend *>(memory_)->handleMMIOReadCompletion(resp);
    } else {
      readData(resp);
    }
    if (!resp->queryFlag(MemEvent::F_NONCACHEABLE))
  ...
  if (ev->queryFlag(PIMMemEvent::F_PIM)) {
    static_cast<PIM::PIMBackend *>(memory_)->handlePIMCompletion(resp);
  } else {
    link_->send(resp);
  }
  delete ev;
...
void MemControllerKG::init(unsigned int phase) {
  link_->init(phase);

  region_ = link_->getRegion(); // This can change during init, but should
                                // stabilize before we start receiving init data

  adjustRegionToMemSize();

  if (!phase) {
    /* Announce our presence on link */
    link_->sendUntimedData(new MemEventInitCoherence(
        getName(), Endpoint::Memory, true, false,
        memBackendConvertor_->getRequestWidth(), false));
    link_->sendUntimedData(new MemEventInitEndpoint(
        getName().c_str(), Endpoint::Memory, region_, true));
  }
  ...
  // fregion_ = flink_->getRegion();  // This can change during init, but should
  // stabilize before we start receiving init data
  ...
  if (!phase) {
    /* Announce our presence on link */
    flink_->sendUntimedData(new MemEventInitCoherence(
        getName(), Endpoint::Memory, true, false,
        memBackendConvertor_->getRequestWidth(), false));
    flink_->sendUntimedData(new MemEventInitEndpoint(
        getName().c_str(), Endpoint::Memory, region_, true));
  }

  while (MemEventInit *ev = flink_->recvUntimedData()) {
    processInitEvent(ev);
  }
}

void MemControllerKG::setup(void) {
...
void MemControllerKG::complete(unsigned int phase) {
...
void MemControllerKG::finish(void) {
  ...
  if (backing_outfile_ != "") {
    try {
      ...
      out.output("%s, WARNING: Unable to open file '%s' provided by parameter "
                 "'backing_out_file' to write memory contents. Memory contents "
                 "will not be written.\n",
                 getName().c_str(), backing_outfile_.c_str());
    }
  }
  if (backing_outscreen_) {
    backing_->printToScreen(privateMemOffset_, region_.start,
                            region_.interleaveSize, region_.interleaveStep);
  }
}

void MemControllerKG::writeData(MemEvent *event) {
  if (event->getCmd() == Command::PutM) { /* Write request to memory */
    Addr addr = event->queryFlag(MemEvent::F_NONCACHEABLE)
                    ? event->getAddr()
                    : event->getBaseAddr();
    if (mem_h_is_debug_event(event)) {
      mem_h_debug_output(_L8_,
                         "S: Update backing. Addr = %" PRIx64 ", Size = %i\n",
                         addr, event->getSize());
...
void MemControllerKG::readData(MemEvent *event) {
...
void MemControllerKG::writeData(Addr addr, std::vector<uint8_t> *data) {
  if (!backing_)
    return;
...
void MemControllerKG::readData(Addr addr, size_t bytes,
...
Addr MemControllerKG::translateToLocal(Addr addr) {
  Addr rAddr = addr;
  ...
  if (mem_h_is_debug_addr(addr) && addr != rAddr) {
    mem_h_debug_output(_L10_,
                       "C: %-40" PRIu64
                       "  %-20s ConvertAddr   Local, 0x%" PRIx64 ", 0x%" PRIx64
                       "\n",
...
Addr MemControllerKG::translateToGlobal(Addr addr) {
  ...
  if (mem_h_is_debug_addr(rAddr) && addr != rAddr) {
    mem_h_debug_output(_L10_,
                       "C: %-40" PRIu64
                       "  %-20s ConvertAddr   Global, 0x%" PRIx64 ", 0x%" PRIx64
                       "\n",
                       getCurrentSimCycle(), getName().c_str(), addr, rAddr);
  }
  return rAddr;
...
void MemControllerKG::processInitEvent(MemEventInit *me) {
  /* Push data to memory */
  if (Command::Write == me->getCmd()) {
    ...
    mem_h_debug_output(_L10_,
                       "U: %-20s   Event:Untimed   UNKNOWN COMMAND (%s)\n",
                       getName().c_str(), me->getVerboseString().c_str());
  }
...
void MemControllerKG::adjustRegionToMemSize() {
  // Check memSize_ against region
  // Set region_ to the smaller of the two
  // It's sometimes useful to be able to adjust one of the params and not the
  // other So, a mismatch is likely not an error, but alert the user in debug
  // mode just in case
  // TODO deprecate mem_size & just use region?
  uint64_t regSize = region_.end - region_.start;
  if (regSize !=
      region_.REGION_MAX) { // The default is for region_.end = uint64_t -1, but
                            // then if we add one we wrap...
  ...
  if (region_.interleaveStep != 0) {
    ...
    if (regSize > memSize_) { /* Reduce the end point so that regSize is no
                                 larger than memSize */
#ifdef __SST_DEBUG_OUTPUT__
      out.output(
          "%s, Notice: memory controller's region is larger than the backend's "
          "mem_size, controller is limiting accessible memory to mem_size\n"
          "Region: start=%" PRIu64 ", end=%" PRIu64 ", interleaveStep=%" PRIu64
          ", interleaveSize=%" PRIu64 ". MemSize: %zuB\n",
          getName().c_str(), region_.start, region_.end, region_.interleaveStep,
          region_.interleaveSize, memSize_);
#endif
...
void MemControllerKG::printStatus(Output &statusOut) {
  statusOut.output("MemHierarchy::MemoryController %s\n", getName().c_str());

  statusOut.output("  Outstanding events: %zu\n", outstandingEvents_.size());
  for (std::map<SST::Event::id_type, MemEventBase *>::iterator it =
           outstandingEvents_.begin();
       it != outstandingEvents_.end(); it++) {
    statusOut.output("    %s\n",
                     it->second->getVerboseString((int)dlevel).c_str());
  }
  ...
  if (flink_)
    flink_->printStatus(statusOut);

  statusOut.output("End MemHierarchy::MemoryController\n\n");
}

void MemControllerKG::emergencyShutdown() {
  if (out.getVerboseLevel() > 1) {
    ...
    if (flink_) {
      out.output("  Checking for unreceived events on forwarding link: \n");
      flink_->emergencyShutdownDebug(out);
    }
  }
}

void MemControllerKG::printDataValue(Addr addr, std::vector<uint8_t> *data,
                                     bool set) {
  if (dlevel < 11)
    return;
  ...
  dbg.debug(_L11_,
            "V: %-20" PRIu64 " %-20" PRIu64 " %-20s %-13s 0x%-16" PRIx64
            " B: %-3zu %s\n",
            getCurrentSimCycle(), getNextClockCycle(clockTimeBase_) - 1,
            getName().c_str(), action.c_str(), addr, data->size(),
            value.str().c_str());
}
 ...
 * Default constructor
 */
MemControllerKG::MemControllerKG() : Component() {}

/*
 * Serialize function
 */
void MemControllerKG::serialize_order(
