...
// These have to be copy from the source area
...
/*************************** Memory Controller ********************/
MemControllerKG::MemControllerKG( ComponentId_t id, Params& params ) : Component( id ), backing_( NULL ) {
  ...
  if( found ) {
    out.output(
      "%s, ** Found deprecated parameter: network_num_vc ** MemHierarchy does "
  ...
  if( found ) {
    assert( false );
    out.output(
      "%s, ** Found deprecated parameter: direct_link ** The value "
  ...
  clockHandler_   = new Clock::Handler<MemControllerKG>( this, &MemControllerKG::clock );
  ...
  if( !memory_ ) { /* Try to load from our parameters (legacy mode 1) */
    ...
    memory_               = loadAnonymousSubComponent<MemBackend>(
      name, "backend", 0, ComponentInfo::INSERT_STATS | ComponentInfo::SHARE_PORTS, tmpParams
    );
    if( !memory_ ) {
      out.fatal(
        CALL_INFO,
  ...
  memBackendConvertor_      = loadAnonymousSubComponent<MemBackendConvertor>(
    convertortype, "backendConvertor", 0, ComponentInfo::INSERT_STATS, tmpParams, memory_, requestWidth
  );
  ...
  memBackendConvertor_->setCallbackHandlers(
    std::bind( &MemControllerKG::handleMemResponse, this, _1, _2 ), std::bind( &MemControllerKG::turnClockOn, this )
  );
    ...
    for( uint32_t i = 0; i < listenerCount; ++i ) {
      ...
      if( listenerMod != "" ) {
        ...
        CacheListener* loadedListener =
          loadAnonymousSubComponent<CacheListener>( listenerMod, "listener", i, ComponentInfo::INSERT_STATS, listenerParams );
  ...
  if( !gotRegion ) {
    out.output(
      "%s, WARNING: Memories no longer inherit address regions from "
  ...
  flink_                 = loadUserSubComponent<MemLinkBase>( "forwardlink", ComponentInfo::SHARE_NONE, clockTimeBase_ );
  ...
  if( !link_ && isPortConnected( "direct_link" ) ) {
    ...
    link_ = loadAnonymousSubComponent<MemLinkBase>(
      "memHierarchy.MemLink", "cpulink", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, linkParams, clockTimeBase_
    );
  } else if( !link_ ) {
    ...
    if( isPortConnected( "network_ack" ) && isPortConnected( "network_fwd" ) && isPortConnected( "network_data" ) ) {
      ...
      link_ = loadAnonymousSubComponent<MemLinkBase>(
        "memHierarchy.MemNICFour", "cpulink", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, nicParams, clockTimeBase_
      );
    } else {
      nicParams.insert( "port", "network" );
      link_ = loadAnonymousSubComponent<MemLinkBase>(
        "memHierarchy.MemNIC", "cpulink", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, nicParams, clockTimeBase_
      );
    }
  }

  clockLink_ = link_->isClocked();
  link_->setRecvHandler( new Event::Handler<MemControllerKG>( this, &MemControllerKG::handleEvent ) );

  link_->setRegion( region_ );

  if( flink_ ) {
    fClockLink_ = flink_->isClocked();
    flink_->setRecvHandler( new Event::Handler<MemControllerKG>( this, &MemControllerKG::handleFLinkEvent ) );
  ...
  std::string backingType =
    params.find<std::string>( "backing", "mmap", found ); /* Default to using an mmap backing store, fall back on malloc */
  backing_ = nullptr;
  if( !found ) {
  ...
  if( backingType == "mmap" ) {
    ...
    try {
      ...
      if( e == 1 )
        out.fatal(
          CALL_INFO, -1, "%s, Error - unable to open memory_file. You specified '%s'.\n", getName().c_str(), memoryFile.c_str()
        );
      else if( e == 2 ) {
        if( memoryFile == "" ) {
          out.verbose(
          ...
          out.fatal(
            CALL_INFO, -1, "%s, Error - Could not MMAP backing store from file %s\n", getName().c_str(), memoryFile.c_str()
          );
        }
      } else
        out.fatal(
  ...
  customCommandHandler_ = loadUserSubComponent<CustomCmdMemHandler>(
    ...
    std::bind(
      static_cast<void ( MemControllerKG::* )( Addr, size_t, std::vector<uint8_t>& )>( &MemControllerKG::readData ),
      this,
      _1,
      _2,
      _3
    ...
    std::bind(
      static_cast<void ( MemControllerKG::* )( Addr, std::vector<uint8_t>* )>( &MemControllerKG::writeData ), this, _1, _2
    )
  );
...
void MemControllerKG::handleEvent( SST::Event* event ) {
  ...
  if( is_debug_event( meb ) ) {
    Debug(
      _L3_,
      "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:New     (%s)\n",
      getCurrentSimCycle(),
      getNextClockCycle( clockTimeBase_ ) - 1,
      getName().c_str(),
      meb->getVerboseString( dlevel ).c_str()
    );
  }
  ...
  if( region_.interleaveStep != 0 &&
      ev->getSize() > 0 ) {  // Non-contiguous address region so make sure this request doesn't fall outside the region
  ...
  switch( cmd ) {
  case Command::PutM: ev->setFlag( MemEvent::F_NORESPONSE );
  case Command::GetS:
  case Command::GetX:
  case Command::GetSX:
  case Command::Write:
  ...
  case Command::PutS:
  case Command::PutE: delete ev; break;
  default: out.fatal( CALL_INFO, -1, "Memory controller received unrecognized command: %s", CommandString[(int) cmd] );
  }
}
...
void SST::MemHierarchy::MemControllerKG::handleForwardedEvent( MemEventBase* ev ) {
...
void MemControllerKG::handleMemResponse( Event::id_type id, uint32_t flags ) {
  ...
  if( it == outstandingEvents_.end() )
    out.fatal(
      CALL_INFO,
      -1,
      "Memory controller (%s) received unrecognized response ID: %" PRIu64 ", %" PRIu32 "",
      getName().c_str(),
      id.first,
      id.second
    );
  ...
  if( is_debug_event( evb ) ) {
    Debug(
      _L4_,
      "B: %-20" PRIu64 " %-20" PRIu64 " %-20s Bkend:Recv    (<%" PRIu64 ",%" PRIu32 ">)\n",
      getCurrentSimCycle(),
      getNextClockCycle( clockTimeBase_ ) - 1,
      getName().c_str(),
      id.first,
  ...
  if( backing_ && ( ev->getCmd() == Command::PutM || ( ev->getCmd() == Command::Write ) ) ) {
    if( ev->queryFlag( PIMMemEvent::F_MMIO ) ) {
      ...
      // TODO carry global and local address information with event so we only translate once.
      if( ev->isAddrGlobal() ) {
  ...
  if( is_debug_event( resp ) ) {
    Debug(
      _L3_,
      "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:Resp    (%s)\n",
      getCurrentSimCycle(),
      getNextClockCycle( clockTimeBase_ ) - 1,
      getName().c_str(),
      resp->getVerboseString( dlevel ).c_str()
    );
  }
...
void MemControllerKG::init( unsigned int phase ) {
  ...
  if( !phase ) {
    ...
    link_->sendUntimedData(
      new MemEventInitCoherence( getName(), Endpoint::Memory, true, false, memBackendConvertor_->getRequestWidth(), false )
    );
  ...
  //fregion_ = flink_->getRegion();  // This can change during init, but should stabilize before we start receiving init data

  //adjustRegionToMemSize();

  if( !phase ) {
    /* Announce our presence on link */
    flink_->sendUntimedData(
      new MemEventInitCoherence( getName(), Endpoint::Memory, true, false, memBackendConvertor_->getRequestWidth(), false )
    );
    flink_->sendUntimedData( new MemEventInitEndpoint( getName().c_str(), Endpoint::Memory, region_, true ) );
  }
...
void MemControllerKG::readData( Addr addr, size_t bytes, std::vector<uint8_t>& data ) {
  data.resize( bytes, 0 );
...
Addr MemControllerKG::translateToLocal( Addr addr ) {
  ...
  if( is_debug_addr( addr ) && addr != rAddr ) {
    Debug(
      _L10_,
      "C: %-40" PRIu64 "  %-20s ConvertAddr   Local, 0x%" PRIx64 ", 0x%" PRIx64 "\n",
      getCurrentSimCycle(),
      getName().c_str(),
      addr,
      rAddr
...
Addr MemControllerKG::translateToGlobal( Addr addr ) {
  ...
  if( is_debug_addr( rAddr ) && addr != rAddr ) {
    Debug(
      _L10_,
      "C: %-40" PRIu64 "  %-20s ConvertAddr   Global, 0x%" PRIx64 ", 0x%" PRIx64 "\n",
      getCurrentSimCycle(),
      getName().c_str(),
      addr,
      rAddr
...
void MemControllerKG::processInitEvent( MemEventInit* me ) {
  ...
  if( Command::Write == me->getCmd() ) {
    ...
    if( is_debug_event( me ) ) {
      Debug(
        _L9_,
        "Memory init %s - Received Write for %" PRIx64 " size %zu\n",
        getName().c_str(),
        me->getAddr(),
        me->getPayload().size()
      );
  ...
  if( region_.interleaveStep != 0 ) {
    ...
    if( regSize > memSize_ ) { /* Reduce the end point so that regSize is no larger than memSize */
      ...
      out.output(
        ...
        region_.start,
        region_.end,
        region_.interleaveStep,
        region_.interleaveSize,
        memSize_
      );
#endif
...
void MemControllerKG::printStatus( Output& statusOut ) {
  ...
  for( std::map<SST::Event::id_type, MemEventBase*>::iterator it = outstandingEvents_.begin(); it != outstandingEvents_.end();
       it++ ) {
...
void MemControllerKG::printDataValue( Addr addr, std::vector<uint8_t>* data, bool set ) {
  if( dlevel < 11 )
    return;
  ...
  dbg.debug(
    _L11_,
    "V: %-20" PRIu64 " %-20" PRIu64 " %-20s %-13s 0x%-16" PRIx64 " B: %-3zu %s\n",
    getCurrentSimCycle(),
    getNextClockCycle( clockTimeBase_ ) - 1,
    getName().c_str(),
    action.c_str(),
