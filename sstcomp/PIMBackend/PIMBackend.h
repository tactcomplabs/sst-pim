...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT(PIMBackend, "PIM", "PIMBackend",
  ...
  virtual void
  setEventInjectionHandler(std::function<void(SST::Event *)> handler) {
    m_pimRequest = handler;
  }

  // Use component name for issuing dram requests. Memhierarchy won't recognize
  // the backend as a source or dest.
  ...
  void issueDRAMRequest(
      uint64_t a, MemEventBase::dataVec *d, bool isWrite,
  ...
  void
  handleMMIOReadCompletion(SST::Event *); // Memory controller gets PIM Data to
                                          // provide in response to network
  void handleMMIOWriteCompletion(
      SST::Event *); // Memory controller writes event payload into PIM
...
// TODO put back
#if 0
    bool isClocked() { return backend->isClocked(); }
#else
  bool isClocked() override { return true; }
#endif

protected:
  SST::Output pimOutput; /// special output stream for PIM different from parent
                         /// class output
...
private:
  ...
  std::map<Event::id_type,
           std::function<void(const MemEventBase::dataVec)>>
      pendingPIMEvents; // id, completion map

  std::map<ReqId, MemEvent *> outstandingPIMReqs;

  std::function<void(SST::Event *)>
      m_pimRequest; // called by backend. memory controller's incoming event
                    // handler
