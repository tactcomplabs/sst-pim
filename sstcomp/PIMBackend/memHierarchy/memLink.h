...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT(MemLink, "memHierarchy", "MemLink",
...
#define MEMLINK_ELI_PARAMS                                                     \
  ...
  virtual void sendUntimedData(MemEventInit *ev, bool broadcast,
...
ImplementSerializable(SST::MemHierarchy::MemLink) protected
  ...
  // Data structures
  std::set<EndpointInfo>
      remotes_; // Tracks remotes (source or dest) immediately accessible on the
                // other side of our link
  std::set<EndpointInfo> peers_; // Tracks peers immediately accessible on the
                                 // other side of our link
  std::set<EndpointInfo> endpoints_; // Tracks endpoints in the system with info
                                     // on how to get there
  std::set<std::string>
      reachable_names_; // Tracks reachable names for faster lookup than
                        // iterating via remotes/peers
  std::set<std::string> peer_names_; // Tracks peer names for faster lookup than
                                     // iterating via peers
