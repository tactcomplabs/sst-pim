 ...
 *
 * This class primarily consists of a Command to perform at a particular
 * address, potentially including data.
 *
 * The command list includes the needed commands to execute cache coherence
 * protocols as well as standard reads and writes to memory.
 */
class MemEvent : public MemEventBase {
public:
  /* Constructor - Coherence control */
  MemEvent(std::string src, Addr addr, Addr baseAddr, Command cmd)
      : MemEventBase(src, cmd) {
    initialize();
    addr_ = addr;
    baseAddr_ = baseAddr;
  }
  ...
  MemEvent *releaseNACKedEvent() {
    return std::exchange(NACKedEvent_, nullptr);
  }
  ...
  bool isReadLock() {
    return cmd_ == Command::GetSX && queryFlag(MemEventBase::F_LOCKED);
  }

  void setLoadLink() { setFlag(MemEventBase::F_LLSC); }
  bool isLoadLink() {
    return cmd_ == Command::GetSX && queryFlag(MemEventBase::F_LLSC);
  }

  void setStoreConditional() { setFlag(MemEventBase::F_LLSC); }
  bool isStoreConditional() {
    return (cmd_ == Command::GetX || cmd_ == Command::Write) &&
           queryFlag(MemEventBase::F_LLSC);
  }

  void setFail() { setFlag(MemEventBase::F_FAIL); }
  void setSuccess(bool b) {
    b ? clearFlag(MemEventBase::F_FAIL) : setFlag(MemEventBase::F_FAIL);
  }
  ...
  size_t getPayloadSize() override { return payload_.size(); }
  ...
  /** Returns true if this is a request that needs to access the data array
   * (Get/Put/Flush) */
  bool isDataRequest(void) const {
    return CommandClassArr[(int)cmd_] == CommandClass::Request;
  }
  /** Returns true if this is of response type */
  bool isResponse(void) const {
    return BasicCommandClassArr[(int)cmd_] == BasicCommandClass::Response;
  }
  /** Returns true if this is a writeback */
  bool isWriteback(void) const { return CommandWriteback[(int)cmd_]; }
  /** Returns true if this request type should be routed by address (versus
   * destination) */
  bool isRoutedByAddress(void) const {
    return CommandRouteByAddress[(int)cmd_];
  }
