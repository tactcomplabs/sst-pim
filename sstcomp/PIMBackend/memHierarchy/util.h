...
#define mem_h_is_debug_addr(addr)                                              \
...
inline bool isPowerOfTwo(unsigned int x) { return !(x & (x - 1)); }
 ...
 */
inline bool fixupParam(SST::Params &params, const std::string oldKey,
                       const std::string newKey) {
  bool found;
  if (params.contains(newKey))
    return false;
...
inline void fixupParams(Params &params, const std::string oldKey,
                        const std::string newKey) {
 ...
 *  BLOCK - this request is blocked by a current outstanding request and should
 * stall in the MSHRs REJECT - this request cannot be handled
 */
