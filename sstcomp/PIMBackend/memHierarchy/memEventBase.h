...
public:
  ...
  [[deprecated( "Use getThreadID() instead (with capital 'D')" )]] const uint32_t getThreadId( void ) const { return tid_; }
  ...
  virtual std::string getVerboseString( int level = 1 ) {
    ...
    return idstring.str() + cmdStr + " Src: " + src_ + " Dst: " + dst_ + " Rq: " + rqstr_ + " Tid: " + std::to_string( tid_ ) +
           str.str();
  }
...
struct memEventCmp {
  bool operator()( const MemEventBase* lhs, const MemEventBase* rhs ) const { return lhs->getID() < rhs->getID(); }
};
...
public:
  ...
  MemEventInitCoherence( std::string src, Endpoint type, bool inclusive, bool sendWBAck, Addr lineSize, bool tracksPresence )
    : MemEventInit( src, InitCommand::Coherence ), type_( type ), inclusive_( inclusive ), sendWBAck_( sendWBAck ),
      recvWBAck_( false ), lineSize_( lineSize ), tracksPresence_( tracksPresence ) {}

  MemEventInitCoherence(
    std::string src, Endpoint type, bool inclusive, bool sendWBAck, bool recvWBAck, Addr lineSize, bool tracksPresence
  )
    : MemEventInit( src, InitCommand::Coherence ), type_( type ), inclusive_( inclusive ), sendWBAck_( sendWBAck ),
      recvWBAck_( recvWBAck ), lineSize_( lineSize ), tracksPresence_( tracksPresence ) {}
  ...
  virtual MemEventInitCoherence* clone( void ) override { return new MemEventInitCoherence( *this ); }
...
public:
  ...
  virtual MemEventInitRegion* clone( void ) override { return new MemEventInitRegion( *this ); }
