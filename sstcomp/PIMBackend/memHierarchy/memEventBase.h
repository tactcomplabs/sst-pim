...
public:
  ...
  const uint32_t getThreadId(void) const {
    return tid_;
  }
  ...
  virtual std::string getVerboseString(int level = 1) {
    ...
    return idstring.str() + cmdStr + " Src: " + src_ + " Dst: " + dst_ +
           " Rq: " + rqstr_ + " Tid: " + std::to_string(tid_) + str.str();
  }
  ...
  virtual bool doDebug(std::set<Addr> &UNUSED(addr)) {
    return true; // Always debug unless we come up with a different way of
                 // determining it
  }
...
protected:
  ...
  id_type
      responseToID_; // For responses, holds the ID to which this event matches
...
public:
  ...
  /* Untimed event */
  MemEventInit(std::string src, InitCommand cmd)
      : MemEventBase(src, Command::NULLCMD), initCmd_(cmd) {}

  /* Untimed memory events that carry data */
  MemEventInit(std::string src, Command cmd, Addr addr,
               std::vector<uint8_t> &data)
      : MemEventBase(src, cmd), initCmd_(InitCommand::Data), addr_(addr),
        size_(0), payload_(data) {}
  ...
  virtual std::string getVerboseString(int level = 1) override {
    ...
    if (initCmd_ == InitCommand::Region)
      str << " InitCmd: Region";
    else if (initCmd_ == InitCommand::Data) {
      str << " InitCmd: Data (0x" << std::hex << addr_ << ", " << std::dec
          << size_ << ")";
    } else if (initCmd_ == InitCommand::Coherence)
      str << " InitCmd: Coherence";
    else if (initCmd_ == InitCommand::Endpoint)
      str << " InitCmd: Endpoint";
    else if (initCmd_ == InitCommand::Flush)
      str << " InitCmd: Flush";
    else
      str << " InitCmd: Unknown command";
...
public:
   ...
   * sendWBAck: the component always sends WB Acks (if false, the component
   * *can* send them if another component needs it) recvWBAck: the component
   * expects to receive WB Acks (if false, the component *can* expect to receive
   * them if another component sends them) lineSize: number of bytes in a line
   * tracksPresence: whether the component keeps track of whether a line is
   * present elsewhere. Affects whether clean evictions need to happen or not.
   */
  MemEventInitCoherence(std::string src, Endpoint type, bool inclusive,
                        bool sendWBAck, Addr lineSize, bool tracksPresence)
      : MemEventInit(src, InitCommand::Coherence), type_(type),
        inclusive_(inclusive), sendWBAck_(sendWBAck), recvWBAck_(false),
        lineSize_(lineSize), tracksPresence_(tracksPresence) {}
  MemEventInitCoherence(std::string src, Endpoint type, bool inclusive,
                        bool sendWBAck, bool recvWBAck, Addr lineSize,
                        bool tracksPresence)
      : MemEventInit(src, InitCommand::Coherence), type_(type),
        inclusive_(inclusive), sendWBAck_(sendWBAck), recvWBAck_(recvWBAck),
        lineSize_(lineSize), tracksPresence_(tracksPresence) {}
  ...
  virtual std::string getVerboseString(int level = 1) override {
    std::ostringstream str;
    str << " Type: " << (int)type_
        << " Inclusive: " << (inclusive_ ? "true" : "false");
    str << " LineSize: " << lineSize_
        << " Tracks presence: " << (tracksPresence_ ? "true" : "false");
...
private:
  ...
  bool sendWBAck_; // Whether endpoint expects writeback acks -> should we send
                   // WB acks to the sender?
  bool recvWBAck_; // Whether endpoint sends writeback acks -> will we receive
                   // WB acks from the sender?
...
public:
   ...
   *
   * TODO: Possibliy merge this with the coherence init messages and broadcast
   * all topology info everywhere
   */
  ...
  void addRegion(MemRegion reg, bool cacheable) {
    regions_.push_back(std::make_pair(reg, cacheable));
  }
  ...
  virtual std::string getVerboseString(int level = 1) override {
    ...
    str << " Type: " << (int)type_ << " Name: " << name_;
    str << " Regions:";
    for (std::vector<std::pair<MemRegion, bool>>::iterator it =
             regions_.begin();
         it != regions_.end(); it++) {
      str << " [" << it->first.toString() << "; "
          << (it->second ? "cacheable" : "noncacheable") << "]";
    }
    return MemEventInit::getBriefString() + str.str();
  }
...
private:
  Endpoint type_;
  std::string name_;
  std::vector<std::pair<MemRegion, bool>>
      regions_; // List of address regions accessible and whether they are
                // cacheable or not
...
public:
  ...
  MemEventInitRegion(std::string src, MemRegion region,
                     ReachableGroup group = ReachableGroup::Unknown)
  ...
  virtual std::string getVerboseString(int level = 1) override {
    ...
    if (group_ == ReachableGroup::Source)
      groupstr = "Source";
    else if (group_ == ReachableGroup::Dest)
      groupstr = "Dest";
    else if (group_ == ReachableGroup::Peer)
      groupstr = "Peer";
    return MemEventInit::getVerboseString(level) + region_.toString() +
           " Group: " + groupstr.c_str();
  }

private:
  MemRegion region_;     // MemRegion for source
  ReachableGroup group_; // Whether sent from a source/dest/peer or something
                         // else (unknown)
...
public:
  ...
  virtual std::string getVerboseString(int level = 1) override {
    return MemEventInit::getVerboseString(level) +
