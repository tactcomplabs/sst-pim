...
uint64_t TCLPIM::getCycle() { return cycle; }
...
unsigned TCLPIM::decodeFuncNum(uint64_t a, unsigned numBytes) {
  // 16 functions
  unsigned n = (a >> 3) & 0xf;
...
void TCLPIM::read(Addr addr, unsigned numBytes, std::vector<uint8_t> &payload) {
  ...
  if (info.pimAccType == PIM_ACCESS_TYPE::SRAM) {
    ...
    output->verbose(CALL_INFO, 3, 0,
                    "PIM 0x%" PRIx64 " IO READ SRAM A=0x%" PRIx64
                    " D=0x%" PRIx64 "\n",
                    id, addr, sramArray[spdIndex]);
  } else {
    auto fnum = decodeFuncNum(addr, numBytes);
    ...
    output->verbose(CALL_INFO, 3, 0,
                    "PIM 0x%" PRIx64 " IO READ FUNC A=0x%" PRIx64
                    " D=0x%" PRIx64 "\n",
                    id, addr, d);
  }
}

void TCLPIM::write(Addr addr, unsigned numBytes,
                   std::vector<uint8_t> *payload) {

  output->verbose(CALL_INFO, 3, 0,
                  "PIM 0x%" PRIx64 " IO WRITE A=0x%" PRIx64 "BYTES=%" PRId32
                  "\n",
                  id, addr, numBytes);
  ...
  if (info.pimAccType == PIM_ACCESS_TYPE::SRAM) {
    ...
    output->verbose(CALL_INFO, 3, 0,
                    "PIM 0x%" PRIx64 " IO WRITE SRAM A=0x%" PRIx64
                    " D=0x%" PRIx64 "\n",
    ...
    output->verbose(CALL_INFO, 3, 0,
                    "PIM 0x%" PRIx64 " IO WRITE FUNC[%d] D=0x%" PRIx64 "\n", id,
                    fnum, data);
  ...
  case FSTATE::READY:
    if (static_cast<FUNC_CMD>(d) == FUNC_CMD::RUN) {
      ...
      parent->output->verbose(
          CALL_INFO, 3, 0,
          "Starting Function[%" PRId32 "]( 0x%" PRIx64 " 0x%" PRIx64
          " 0x%" PRIx64 "0x%" PRIx64 " 0x%" PRIx64 " 0x%" PRIx64 " 0x%" PRIx64
          " 0x%" PRIx64 " )\n",
          static_cast<int>(fnum), params[0], params[1], params[2], params[3],
...
uint64_t TCLPIM::FuncState::readFSM() { return static_cast<uint64_t>(fstate); }

bool TCLPIM::FuncState::running() { return fstate == FSTATE::RUNNING; }
...
void TCLPIM::FuncState::setFSM(std::shared_ptr<FSM> fsm) { exec_ = fsm; }
