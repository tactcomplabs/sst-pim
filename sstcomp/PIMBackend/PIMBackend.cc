...
/*------------------------------- Simple Backend -------------------------------
 */
PIMBackend::PIMBackend(ComponentId_t id, Params &params)
  ...
  backend->setResponseHandler(
      std::bind(&PIMBackend::handleMemResponse, this, _1));
  ...
  if (delay.getValue() != 0) {
    delay_self_link = configureSelfLink(
        ...
        new Event::Handler<PIMBackend>(this, &PIMBackend::handleNextRequest));
  } else {
  ...
  if (pim_type == PIM_TYPE_TEST) {
    ...
    pimOutput.verbose(CALL_INFO, 1, 0,
                      "Warning: no PIM specified (pim_type=0)\n");
    pimsim = nullptr;
  }
  ...
  if (pimsim)
    pimsim->setCallback(
        std::bind(&PIMBackend::issueDRAMRequest, this, _1, _2, _3, _4));
...
bool PIMBackend::issueRequest(ReqId req, Addr addr, bool isWrite,
                              unsigned numBytes) {
...
bool PIMBackend::clock(Cycle_t cycle) {
  ...
  if (pimsim && !initDRAMDone) {
    ...
    MemEventBase::dataVec wrData = {0x10, 0x32, 0x54, 0x76,
                                    0x98, 0xba, 0xdc, 0xfe};
    MemEvent *evw =
        new MemEvent(getName(), spdBase + 64, spdBase + 64, PIM_WRITE, wrData);
    evw->setFlags(MemEvent::F_NONCACHEABLE | MemEvent::F_NORESPONSE);
    m_pimRequest(evw);

    // Read it back. The first read response will be check to clear self-test
    // done.
    MemEventBase::dataVec rdData = {0};
    rdData.resize(8);
    MemEvent *evr =
        new MemEvent(getName(), spdBase + 64, spdBase + 64, PIM_READ, rdData);
...
void PIMBackend::issueDRAMRequest(
    uint64_t a, MemEventBase::dataVec *vec, bool isWrite,
    std::function<void(const MemEventBase::dataVec &)> completion) {
...
void PIMBackend::handlePIMCompletion(SST::Event *resp) {
  ...
  if (it == pendingPIMEvents.end()) {
    pimOutput.fatal(CALL_INFO, -1,
                    "Could not match ID for PIM returning PIM event [ %s ]\n",
                    mev->toString().c_str());
  }
...
void PIMBackend::handleMMIOReadCompletion(SST::Event *ev) {
  ...
  pimOutput.verbose(CALL_INFO, 3, 0,
                    "MMIO read a=0x%" PRIx64 "d[0]=%" PRId32 "\n",
...
void PIMBackend::handleMMIOWriteCompletion(SST::Event *ev) {
  ...
  // TODO: Fix elf / linker / loader to not write initial values to MMIO ranges
  // to avoid side effects
  if (mev->getSize() != 8) {
    pimOutput.verbose(CALL_INFO, 3, 0,
                      "Warning: Dropping MMIO write to function handler with "
                      "numBytes=%" PRIx32 "\n",
                      mev->getSize());
    return;
  }
  ...
  pimOutput.verbose(CALL_INFO, 3, 0,
                    "MMIO write a=0x%" PRIx64 " d[0]=%" PRId32 "\n",
                    mev->getAddr(), (int)buffer[0]);
}

void PIMBackend::setup() { backend->setup(); }

void PIMBackend::finish() { backend->finish(); }
