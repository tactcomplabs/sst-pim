...
void LFSR::start( uint64_t params[NUM_FUNC_PARAMS] ) {
  this->rand_base_addr    = params[0];
  this->seed   = params[1];
  this->len    = params[2];

  assert(PIMDecoder::decode(rand_base_addr).pimAccType == PIM_ACCESS_TYPE::SRAM);

  lfsr = seed;
  samples = 0;
  cycles = 0;

  loop_state   = LOOP_STATE::CYCLING;
  parent->output->verbose(
    CALL_INFO, 3, 0, 
    "LFSR: rand_base_addr=0x%" PRIx64 " seed=0x%" PRIx64 " len=%" PRIu64 "\n", 
    rand_base_addr,seed,len);
}
...
bool LFSR::clock() {
  ...
  if( loop_state == LOOP_STATE::CYCLING ) {
    ...
    const uint64_t lfsr_next = (lfsr() >> 1) | (bit << 63);
    lfsr = lfsr_next;

    // write sample if cycle
    const bool save_sample = cycles() >= (13-1);
    cycles = save_sample ? 0 : cycles() + 1;
    if(save_sample){
      MemEventBase::dataVec lfsr_vec = MemEventBase::dataVec(sizeof(uint64_t));
      std::memcpy(lfsr_vec.data(),&lfsr_next,sizeof(uint64_t));
      parent->write(rand_base_addr + (samples() * sizeof(uint64_t)),sizeof(uint64_t),&lfsr_vec);
      samples = samples() + 1;
    }

    // exit after all samples written
    const bool all_samples_written = (samples() == (len - 1)) && save_sample;
    if(all_samples_written){
