...
void MemCopy::start(uint64_t params[NUM_FUNC_PARAMS]) {
  unsigned numBytes = (unsigned)params[2];
  if (numBytes == 0)
    return;
  assert((numBytes % 8) == 0);
  dst = params[0];
  ...
  parent->output->verbose(CALL_INFO, 3, 0,
                          "start dma: dst=0x%" PRIx64 " src=0x%" PRIx64
                          " total_words=%" PRId32 "\n",
                          dst, src, total_words);
  ...
  if (dst_inf.isIO && (dst_inf.pimAccType != PIM_ACCESS_TYPE::SRAM))
    parent->output->fatal(CALL_INFO, -1,
                          "Destination address must by SRAM or DRAM\n");
  if (src_inf.isIO && (src_inf.pimAccType != PIM_ACCESS_TYPE::SRAM))
    parent->output->fatal(CALL_INFO, -1,
                          "Source address must by SRAM or DRAM\n");
...
bool MemCopy::clock() {
  ...
  case DMA_STATE::WRITE: {
    ...
    DMA_STATE nextState =
        (word_counter > 0) ? DMA_STATE::READ : DMA_STATE::DONE;
    if (dst_is_sram) {
...
void MemCopy::sequence_dram_read(DMA_STATE nextState) {
  parent->m_issueDRAMRequest(src, &parent->buffer, false,
                             [this, nextState](const MemEventBase::dataVec &d) {
...
void MemCopy::sequence_dram_write(DMA_STATE nextState) {
  parent->m_issueDRAMRequest(dst, &parent->buffer, true,
                             [this, nextState](const MemEventBase::dataVec &d) {
                               dma_state = nextState;
                             });
}
