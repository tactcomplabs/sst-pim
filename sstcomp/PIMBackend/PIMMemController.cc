...
PIMMemController::PIMMemController(ComponentId_t id, Params &params)
    : MemControllerKG(id, params) {
  PIMBackend *backend = static_cast<PIMBackend *>(memory_);
  backend->setComponentName(
      getName()); // used to generate src identifier for requests
  using std::placeholders::_1;
  backend->setEventInjectionHandler(
      std::bind(&MemControllerKG::handlePIMEvent, this, _1));
...
void PIMMemController::setup() { MemControllerKG::setup(); }
...
void PIMMemController::handleMemResponse(SST::Event::id_type id,
                                         uint32_t flags) {
