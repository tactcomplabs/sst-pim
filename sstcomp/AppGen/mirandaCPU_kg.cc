...
RequestGenCPU_KG::RequestGenCPU_KG( SST::ComponentId_t id, SST::Params& params )
  ...
  out->verbose(
    ...
    1,
    0,
    "Configured CPU to allow %" PRIu32 " maximum Load requests to be memory to be outstanding.\n",
  ...
  out->verbose(
    ...
    1,
    0,
    "Configured CPU to allow %" PRIu32 " maximum Store requests to be memory to be outstanding.\n",
  ...
  out->verbose(
    ...
    1,
    0,
    "Configured CPU to allow %" PRIu32 " maximum Custom requests to be memory to be outstanding.\n",
    maxRequestsPending[CUSTOM]
  );

  std::string cpuClock = params.find<std::string>( "clock", "2GHz" );
  clockHandler         = new Clock::Handler<RequestGenCPU_KG>( this, &RequestGenCPU_KG::clockTick );
  ...
  cache_link = loadUserSubComponent<Interfaces::StandardMem>(
    ...
    timeConverter,
    new Interfaces::StandardMem::Handler<RequestGenCPU_KG>( this, &RequestGenCPU_KG::handleEvent )
  );
  if( !cache_link ) {
  ...
  const uint64_t pageSize  = params.find<uint64_t>( "pagesize", 0xffffffffffff0000 );
  ...
  if( reqGen ) {
    ...
    if( !reqGenModName.empty() ) {
      ...
      srcLink = configureLink( "src", "50ps", new Event::Handler<RequestGenCPU_KG>( this, &RequestGenCPU_KG::handleSrcEvent ) );
      if( NULL == srcLink ) {
...
RequestGenCPU_KG::~RequestGenCPU_KG() {
  delete out;
}

void RequestGenCPU_KG::finish() {}

void RequestGenCPU_KG::init( unsigned int phase ) {
...
void RequestGenCPU_KG::loadGenerator( const std::string& name, SST::Params& params ) {
...
void RequestGenCPU_KG::StdMemHandler::handle( Interfaces::StandardMem::ReadResp* rsp ) {
...
void RequestGenCPU_KG::StdMemHandler::handle( Interfaces::StandardMem::WriteResp* rsp ) {
  cpu->requestsPending[WRITE]--;
}

void RequestGenCPU_KG::StdMemHandler::handle( Interfaces::StandardMem::CustomResp* rsp ) {
...
void RequestGenCPU_KG::issueCustomRequest( CustomOpRequest* req ) {
  ...
  out->verbose(
    CALL_INFO, 4, 0, "Issue request: address=0x%" PRIx64 ", size=%" PRIu64 ", operation=%s\n", reqAddress, reqLength, "CUSTOM"
  );
...
void RequestGenCPU_KG::issueRequest( MemoryOpRequest* req ) {
  ...
  if( lineOffset + reqLength > cacheLine ) {
    ...
    if( isRead ) {
      ...
      // build a normal event
      std::vector<uint8_t> data( lowerLength, write_data );
...
bool RequestGenCPU_KG::clockTick( SST::Cycle_t cycle ) {
  ...
  if( reqGen->isFinished() ) {
    if( ( pendingRequests.size() == 0 ) && ( 0 == requestsPending[READ] ) && ( 0 == requestsPending[WRITE] ) && ( 0 == requestsPending[CUSTOM] ) ) {
      out->verbose(
        CALL_INFO,
        4,
        0,
      ...
      if( requestsPending[memOpReq->getOperation()] < maxRequestsPending[memOpReq->getOperation()] ) {
        ...
        if( nxtRq->canIssue() ) {
          ...
          out->verbose(
            ...
            3,
            0,
            "Request %" PRIu64 " in queue, has dependencies which are not satisfied, wait.\n",
            nxtRq->getRequestID()
          );
        }
      } else {
