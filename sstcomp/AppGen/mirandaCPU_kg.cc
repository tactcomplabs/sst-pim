...
std::atomic<uint64_t> SST::AppGen::GeneratorRequest::nextGeneratorRequestID(0);

using namespace SST::AppGen;

RequestGenCPU_KG::RequestGenCPU_KG(SST::ComponentId_t id, SST::Params &params)
    : Component(id), reqGen(NULL), srcLink(NULL) {
  kgdbg::spinner("MIRANDA_SPINNER");
  const uint32_t verbose = params.find<uint32_t>("verbose", 0);
  std::stringstream prefix;
  prefix << "@t:" << getName() << ":RequestGenCPU_KG[@p:@l]: ";
  ...
  maxRequestsPending[WRITE] =
      params.find<uint32_t>("maxstorememreqpending", 16);
  maxRequestsPending[CUSTOM] =
      params.find<uint32_t>("maxcustommemreqpending", 16);

  requestsPending[READ] = requestsPending[WRITE] = requestsPending[CUSTOM] = 0;

  out->verbose(CALL_INFO, 1, 0,
               "Configured CPU to allow %" PRIu32
               " maximum Load requests to be memory to be outstanding.\n",
               maxRequestsPending[READ]);
  out->verbose(CALL_INFO, 1, 0,
               "Configured CPU to allow %" PRIu32
               " maximum Store requests to be memory to be outstanding.\n",
               maxRequestsPending[WRITE]);
  out->verbose(CALL_INFO, 1, 0,
               "Configured CPU to allow %" PRIu32
               " maximum Custom requests to be memory to be outstanding.\n",
               maxRequestsPending[CUSTOM]);

  std::string cpuClock = params.find<std::string>("clock", "2GHz");
  clockHandler =
      new Clock::Handler<RequestGenCPU_KG>(this, &RequestGenCPU_KG::clockTick);
  timeConverter = registerClock(cpuClock, clockHandler);

  out->verbose(CALL_INFO, 1, 0, "CPU clock configured for %s\n",
               cpuClock.c_str());

  // program arguments for applink
  // placeholder for argv[0] from c calling convention
  appLink.args.push_back("appLink");
  std::string s = params.find<std::string>("args", "");
  // see RevLoader::splitStr
  char c = ' ';
  std::string::size_type i = 0;
  std::string::size_type j = s.find(c);
  // catch strings with no delims
  if (j == std::string::npos)
    appLink.args.push_back(s);
  // break up the rest of the string
  while (j != std::string::npos) {
    appLink.args.push_back(s.substr(i, j - i));
    i = ++j;
    j = s.find(c, j);
    if (j == std::string::npos)
      appLink.args.push_back(s.substr(i, s.length()));
  }

  //
  // cache_link
  //
  cache_link = loadUserSubComponent<Interfaces::StandardMem>(
      "memory", ComponentInfo::SHARE_NONE, timeConverter,
      new Interfaces::StandardMem::Handler<RequestGenCPU_KG>(
          this, &RequestGenCPU_KG::handleEvent));
  if (!cache_link) {
    std::string interfaceName = params.find<std::string>(
        "memoryinterface", "memHierarchy.standardInterface");
    ...
    cache_link = loadAnonymousSubComponent<Interfaces::StandardMem>(
        interfaceName, "memory", 0,
  ...
  // if( useAppLink )
  maxOpLookup = params.find<uint32_t>("max_reorder_lookups", 16);
  // else
  //   maxOpLookup = params.find< uint64_t >( "max_reorder_lookups", 1 );
...
#else
  // TODO how to have page size be all of 64 bit memory space
  const uint64_t pageSize =
      params.find<uint64_t>("pagesize", 0xffffffffffff0000);
  const uint64_t pageCount = params.find<uint64_t>("pagecount", 1);
#endif
  ...
  if (reqGen) {
    ...
    if (!reqGenModName.empty()) {
      ...
      reqGen = loadAnonymousSubComponent<RequestGenerator>(
          reqGenModName, "generator", 0, ComponentInfo::INSERT_STATS,
          genParams);
      if (NULL == reqGen) {
      ...
      assert(false); // not supported right?
      ...
      srcLink = configureLink("src", "50ps",
                              new Event::Handler<RequestGenCPU_KG>(
                                  this, &RequestGenCPU_KG::handleSrcEvent));
      if (NULL == srcLink) {
  ...
  out->verbose(CALL_INFO, 1, 0,
               "- Max requests per cycle:         %" PRIu32 "\n",
               reqMaxPerCycle);
  out->verbose(CALL_INFO, 1, 0,
               "- Max reorder lookups             %" PRIu32 "\n", maxOpLookup);
  ...
  out->verbose(CALL_INFO, 1, 0,
               ...
               cacheLine);
  out->verbose(CALL_INFO, 1, 0,
               "- Max Load requests pending:      %" PRIu32 "\n",
               maxRequestsPending[READ]);
  out->verbose(CALL_INFO, 1, 0,
               "- Max Store requests pending:     %" PRIu32 "\n",
               maxRequestsPending[WRITE]);
  out->verbose(CALL_INFO, 1, 0,
               "- Max Custom requests pending:     %" PRIu32 "\n",
               maxRequestsPending[CUSTOM]);
  out->verbose(CALL_INFO, 1, 0, "Configuration completed.\n");
}

RequestGenCPU_KG::~RequestGenCPU_KG() { delete out; }

void RequestGenCPU_KG::finish() {}

void RequestGenCPU_KG::init(unsigned int phase) { cache_link->init(phase); }

void RequestGenCPU_KG::handleSrcEvent(Event *ev) {
...
void RequestGenCPU_KG::loadGenerator(MirandaReqEvent *event) {
...
void RequestGenCPU_KG::loadGenerator(const std::string &name,
                                     SST::Params &params) {
  ...
  reqGen = loadAnonymousSubComponent<RequestGenerator>(
      name, "generator", 0, ComponentInfo::SHARE_NONE, params);
...
void RequestGenCPU_KG::handleEvent(Interfaces::StandardMem::Request *ev) {
  out->verbose(CALL_INFO, 3, 0, "Recv event for processing from interface\n");

  Interfaces::StandardMem::Request::id_t reqID = ev->getID();
  std::map<Interfaces::StandardMem::Request::id_t, CPURequest *>::iterator
      reqFind = requestsInFlight.find(reqID);

  if (reqFind == requestsInFlight.end()) {
    out->fatal(CALL_INFO, -1,
               "Unable to find request %" PRIu64 " in request map.\n", reqID);
  } else {
    CPURequest *cpuReq = reqFind->second;

    out->verbose(CALL_INFO, 4, 0,
                 "Miranda request located ID=%" PRIu64 ", contains %" PRIu32
                 " parts, issue time=%" PRIu64 ", time now=%" PRIu64 "\n",
                 cpuReq->getOriginalReqID(), cpuReq->countParts(),
                 cpuReq->getIssueTime(), getCurrentSimTimeNano());
    ...
    // If all the parts of this request are now completed then we will mark it
    // for deletion and update any pending requests which are depending on us
    if (cpuReq->completed()) {
      out->verbose(CALL_INFO, 4, 0,
                   "-> Entry has all parts satisfied, removing ID=%" PRIu64
                   ", total processing time: %" PRIu64 "ns\n",
                   cpuReq->getOriginalReqID(),
                   (getCurrentSimTimeNano() - cpuReq->getIssueTime()));
...
void RequestGenCPU_KG::StdMemHandler::handle(
    Interfaces::StandardMem::ReadResp *rsp) {
  cpu->requestsPending[READ]--;
  if (cpu->useAppLink) {
    // std::vector<uint8_t> data
    assert(rsp->data.size() == 8);
    uint64_t d = 0;
    for (size_t i = 0; i < 8; i++) {
      d |= ((uint64_t)rsp->data[i]) << (i * 8);
    }
    ((AppTransactor *)cpu->reqGen)->handleLoadResponse(d);
  }
}

void RequestGenCPU_KG::StdMemHandler::handle(
    Interfaces::StandardMem::WriteResp *rsp) {
  cpu->requestsPending[WRITE]--;
}

void RequestGenCPU_KG::StdMemHandler::handle(
    Interfaces::StandardMem::CustomResp *rsp) {
  cpu->requestsPending[CUSTOM]--;
  // The CustomResp destructor does not delete the data
  // Do not need to delete the cpuReq data as the memory system should take care
  // of that
  delete rsp->data;
}

void RequestGenCPU_KG::issueCustomRequest(CustomOpRequest *req) {
  const uint64_t reqAddress = req->getPayload()->getRoutingAddress();
  const uint64_t reqLength = req->getPayload()->getSize();
  out->verbose(CALL_INFO, 4, 0,
               "Issue request: address=0x%" PRIx64 ", size=%" PRIu64
               ", operation=%s\n",
  ...
  Interfaces::StandardMem::CustomReq *request =
      new Interfaces::StandardMem::CustomReq(req->getPayload());
  ...
  requestsInFlight.insert(
      std::pair<Interfaces::StandardMem::Request::id_t, CPURequest *>(
          request->getID(), newCPUReq));
...
void RequestGenCPU_KG::issueRequest(MemoryOpRequest *req) {
  ...
  uint64_t write_data = useAppLink ? req->getData() : 0;
  ...
  if (lineOffset + reqLength > cacheLine) {
    ...
    const uint64_t upperAddress = memMgr->mapAddress(
        (lowerAddress - lowerAddress % cacheLine) + cacheLine);

    out->verbose(CALL_INFO, 4, 0, "Issuing a split cache line operation:\n");
    out->verbose(CALL_INFO, 4, 0,
                 "L -> Address: %" PRIu64 ", Length=%" PRIu64 "\n",
                 lowerAddress, lowerLength);
    out->verbose(CALL_INFO, 4, 0,
                 "U -> Address: %" PRIu64 ", Length=%" PRIu64 "\n",
    ...
    if (isRead) {
      ...
      // build a normal event
      std::vector<uint8_t> data(lowerLength, (uint8_t)write_data);
      reqLower =
          new Interfaces::StandardMem::Write(lowerAddress, lowerLength, data);
      data.resize(upperLength, 0);
      reqUpper =
          new Interfaces::StandardMem::Write(upperAddress, upperLength, data);
    }
    ...
    requestsInFlight.insert(
        std::pair<Interfaces::StandardMem::Request::id_t, CPURequest *>(
            reqLower->getID(), newCPUReq));
    requestsInFlight.insert(
        std::pair<Interfaces::StandardMem::Request::id_t, CPURequest *>(
            reqUpper->getID(), newCPUReq));
    ...
    if (isRead) {
      ...
      if (useAppLink) {
        ...
        data.resize(8);
        for (size_t i = 0; i < 8; i++) {
          data[i] = (write_data >> (i * 8)) & 0x0ff;
        }
      } else {
        data = std::vector<uint8_t>(reqLength, 0x0);
      }
      request = new Interfaces::StandardMem::Write(addr, reqLength, data, false,
                                                   0, addr);
    }
...
bool RequestGenCPU_KG::clockTick(SST::Cycle_t cycle) {
  // TODO
  if (!appSpawned) {
    ((AppTransactor *)reqGen)->spawnApp(&appLink);
    appSpawned = true;
  }
  ...
  if (reqGen->isFinished()) {
    if ((pendingRequests.size() == 0) && (0 == requestsPending[READ]) &&
        (0 == requestsPending[WRITE]) && (0 == requestsPending[CUSTOM])) {
      out->verbose(CALL_INFO, 4, 0,
                   "Request generator complete and no requests pending, "
                   "simulation can halt.\n");
  ...
  out->verbose(CALL_INFO, 4, 0,
               "Load Requests pending %" PRIu32 ", maximum permitted %" PRIu32
               ".\n",
               requestsPending[READ], maxRequestsPending[READ]);
  out->verbose(CALL_INFO, 4, 0,
               "Store Requests pending %" PRIu32 ", maximum permitted %" PRIu32
               ".\n",
               requestsPending[WRITE], maxRequestsPending[WRITE]);
  out->verbose(CALL_INFO, 4, 0,
               "Custom Requests pending %" PRIu32 ", maximum permitted %" PRIu32
               ".\n",
  ...
  // We need to generate at least as many requests as can be looked up in the
  // OoO window otherwise the issue will have starvation.
  for (unsigned i = pendingRequests.size(); i < maxOpLookup; ++i) {
    if (reqGen->isFinished()) {
      break;
    } else {
      if (!useAppLink || (requestsPending[READ] == 0))
  ...
  for (uint32_t i = 0; i < pendingRequests.size(); ++i) {
    ...
    if (i == maxOpLookup) {
      out->verbose(CALL_INFO, 2, 0,
                   "Hit maximum reorder limit this cycle, no further "
                   "operations will issue.\n");
    ...
    if (nxtRq->getOperation() == REQ_FENCE) {
      if (0 == requestsInFlight.size()) {
        out->verbose(CALL_INFO, 4, 0,
                     "Fence operation completed, no pending requests, will be "
                     "retired.\n");
        ...
        out->verbose(
            CALL_INFO, 4, 0,
            "Fence operation in flight (>0 pending requests), stall.\n");
      }
      ...
      if (requestsPending[CUSTOM] < maxRequestsPending[CUSTOM]) {
        out->verbose(
            CALL_INFO, 4, 0,
            "Will attempt to issue as free slots in the load/store unit.\n");
        ...
        if (nxtRq->canIssue()) {
          ...
          out->verbose(CALL_INFO, 4, 0,
                       "Request %" PRIu64
                       " encountered, cleared to be issued, %" PRIu32
                       " issued this cycle.\n",
      ...
      if (requestsPending[memOpReq->getOperation()] <
          maxRequestsPending[memOpReq->getOperation()]) {
        out->verbose(
            CALL_INFO, 4, 0,
            "Will attempt to issue as free slots in the load/store unit.\n");
        ...
        if (nxtRq->canIssue()) {
          ...
          out->verbose(
              CALL_INFO, 3, 0,
              "Request %" PRIu64
              " in queue, has dependencies which are not satisfied, wait.\n",
              nxtRq->getRequestID());
        }
      } else {
        out->verbose(CALL_INFO, 3, 0,
                     "All load/store/custom slots occupied, no more issues "
                     "will be attempted.\n");
        break;
      }
    } else {
  ...
  if (issued) {
    ...
    out->verbose(CALL_INFO, 4, 0,
                 "Will not issue, not free slots in load/store unit.\n");
