...
AppTransactor::AppTransactor(ComponentId_t id, Params &params)
...
void AppTransactor::generate(MirandaRequestQueue<GeneratorRequest *> *q) {
  ...
  case SRAM_CMD::DONE:
    ...
    break;
  case SRAM_CMD::READ:
    q->push_back(new MemoryOpRequest(e.address, 8, ReqOperation::READ));
    break;
  case SRAM_CMD::WRITE:
    q->push_back(
        new MemoryOpRequest(e.address, 8, ReqOperation::WRITE, e.data));
    break;
  case SRAM_CMD::NOP:
    break;
  default:
    out->fatal(CALL_INFO, -1, "Undefined Cmd %d", static_cast<int>(e.cmd));
    break;
  }
...
bool AppTransactor::isFinished() { return appLink->done; }
