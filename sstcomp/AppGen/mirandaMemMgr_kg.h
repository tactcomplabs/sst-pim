...
public:
  MirandaMemoryManager(
    SST::Output* mgrOutput, const uint64_t _pageSize, const uint64_t _pageCount, const MirandaPageMappingPolicy mapPolicy
  )
    : pageSize( _pageSize ), pageCount( _pageCount ), maxMemoryAddress( _pageSize * _pageCount ), output( mgrOutput ) {

    output->verbose(
      CALL_INFO,
      2,
      0,
      "Creating memory manager, page size=%" PRIu64 ", page count=%" PRIu64 ", max address=%" PRIu64 "\n",
      pageSize,
      pageCount,
      maxMemoryAddress
    );
    ...
    if( order == 0 ) {
      ...
      case RANDOMIZED:
        ...
        for( uint64_t i = 0; i < ( pageCount * 2 ); ++i ) {
          ...
          if( selectA != selectB ) {
            ...
            output->verbose(
              ...
              64,
              0,
              "Swapping index %" PRIu64 " with index %" PRIu64 ", pageA=%" PRIu64 ", pageB=%" PRIu64 "\n",
              selectA,
              selectB,
              pageA,
              pageArr[selectA]
        ...
        for( uint64_t i = 0; i < pageCount; ++i ) {
          output->verbose(
            CALL_INFO, 32, 0, "Virtual Start = %20" PRIu64 " Physical Start = %20" PRIu64 "\n", ( i * pageSize ), pageArr[i]
          );
        }
  ...
  uint64_t mapAddress( const uint64_t addrIn ) const {
    if( __builtin_expect( addrIn >= maxMemoryAddress, 0 ) ) {
      output->fatal(
        CALL_INFO,
        -1,
        "Error: address %" PRIu64 " exceeds the maximum address in Miranda calculated by "
        "pageSize (%" PRIu64 ") * pageCount (%" PRIu64 ") = %" PRIu64 "\n",
        addrIn,
        pageSize,
        pageCount,
        maxMemoryAddress
...
#define MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL 16
    ...
    if( __builtin_expect( output->getVerboseLevel() >= MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0 ) ) {
      output->verbose(
        ...
        0,
        "Mapping v-addr: %" PRIu64 " pageOffset: %" PRIu64 " -> (%" PRIu64 " - %" PRIu64 ")/%" PRIu64 " = v-page-index: %" PRIu64
        ", p-start: %" PRIu64 " + poffset %" PRIu64 " = %" PRIu64 "\n",
        addrIn,
        pageOffset,
        addrIn,
        pageOffset,
