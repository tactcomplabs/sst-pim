...
#ifndef _H_SST_APPGEN_MIRANDA_MEMORY_MANAGER
#define _H_SST_APPGEN_MIRANDA_MEMORY_MANAGER
...
enum MirandaPageMappingPolicy { LINEAR, RANDOMIZED };

class MirandaMemoryManager {

public:
  MirandaMemoryManager(SST::Output *mgrOutput, const uint64_t _pageSize,
                       const uint64_t _pageCount,
                       const MirandaPageMappingPolicy mapPolicy)
      : pageSize(_pageSize), pageCount(_pageCount),
        maxMemoryAddress(_pageSize * _pageCount), output(mgrOutput) {

    output->verbose(CALL_INFO, 2, 0,
                    "Creating memory manager, page size=%" PRIu64
                    ", page count=%" PRIu64 ", max address=%" PRIu64 "\n",
    ...
    if (order == 0) {
      ...
      case LINEAR:
        output->verbose(CALL_INFO, 2, 0,
                        "Memory is set to LINEAR mapping, will not adjust "
                        "current page maps\n");
        // Nothing to do
        break;
      ...
      case RANDOMIZED:
        output->verbose(CALL_INFO, 2, 0,
                        "Memory is set to RANDOMIZED mapping, will perform a "
                        "randomized shuffle of pages...\n");
        ...
        for (uint64_t i = 0; i < (pageCount * 2); ++i) {
          ...
          if (selectA != selectB) {
            ...
            output->verbose(CALL_INFO, 64, 0,
                            "Swapping index %" PRIu64 " with index %" PRIu64
                            ", pageA=%" PRIu64 ", pageB=%" PRIu64 "\n",
        ...
        for (uint64_t i = 0; i < pageCount; ++i) {
          output->verbose(CALL_INFO, 32, 0,
                          "Virtual Start = %20" PRIu64
                          " Physical Start = %20" PRIu64 "\n",
  ...
  uint64_t mapAddress(const uint64_t addrIn) const {
    if (__builtin_expect(addrIn >= maxMemoryAddress, 0)) {
      output->fatal(CALL_INFO, -1,
                    "Error: address %" PRIu64
                    " exceeds the maximum address in Miranda calculated by "
                    "pageSize (%" PRIu64 ") * pageCount (%" PRIu64
                    ") = %" PRIu64 "\n",
    ...
    // Explicitely casting index to 32 bits to indicate loss of precision.
    // TODO Could we have more than 2**32 pages?
    const uint64_t physPageStart = pageMap[(int)virtualPageStart];
    const uint64_t physAddress = physPageStart + pageOffset;

#define MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL 16

    if (__builtin_expect(output->getVerboseLevel() >=
                             MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL,
                         0)) {
      output->verbose(CALL_INFO, MIRANDA_MAP_ADDRESS_VERBOSE_LEVEL, 0,
                      "Mapping v-addr: %" PRIu64 " pageOffset: %" PRIu64
                      " -> (%" PRIu64 " - %" PRIu64 ")/%" PRIu64
                      " = v-page-index: %" PRIu64 ", p-start: %" PRIu64
                      " + poffset %" PRIu64 " = %" PRIu64 "\n",
                      addrIn, pageOffset, addrIn, pageOffset, pageSize,
                      virtualPageStart, physPageStart, pageOffset, physAddress);
    }
...
#endif //_H_SST_APPGEN_MIRANDA_MEMORY_MANAGER
